...
projects/
  <project-id>/
    ...
    market-research/
      ...
      report.md
      sources.json
    _manifest.json   # which skills have run, in what order, timestamps
```

## V2 upgrade path (not built yet)
Replace the fixed table with an LLM planning step that reads each skill's
`manifest.yaml` (id, description, depends_on) and proposes an execution
plan, which the user can approve/edit before it runs. Keep the artifact
validation and "stop and ask" rules unchanged — those are architecture,
not routing logic.
